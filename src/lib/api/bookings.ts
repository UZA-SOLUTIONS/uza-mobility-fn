...
export function adminListBookings(params?: {
  ...
  if (params?.page) searchParams.set('page', String(params.page));
  if (params?.limit) searchParams.set('limit', String(params.limit));
  return authenticatedPaginatedFetch<VehicleBooking>('/admin/bookings', {
    searchParams,
  });
}

export function adminConfirmBooking(id: string) {
  return authenticatedFetch<VehicleBooking>(`/admin/bookings/${id}/confirm`, {
    method: 'PATCH',
  });
}

export function adminRejectBooking(id: string, body?: { reason?: string }) {
  return authenticatedFetch<VehicleBooking>(`/admin/bookings/${id}/reject`, {
    method: 'PATCH',
    body: JSON.stringify(body ?? {}),
  });
}
